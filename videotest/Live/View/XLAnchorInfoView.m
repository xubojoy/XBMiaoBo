...
- (void)setHotModel:(LiveListModel *)hotModel
{
    _hotModel = hotModel;
...
//    }
    ...
    self.peopleLabel.text = [NSString stringWithFormat:@"%d人", hotModel.allnum];
    ...
    self.roomID.text = [NSString stringWithFormat:@"房间号:%d",hotModel.roomid];
}
...
- (void)updateNum
{
    ...
    self.peopleLabel.text = [NSString stringWithFormat:@"%d人", self.hotModel.allnum + randomNum];
...
- (void)setAllModels:(NSArray *)allModels
{
    ...
    self.peoplesScrollView.contentSize = CGSizeMake((self.peoplesScrollView.height + margin) * self.allModels.count + margin, 0);
    ...
    for (int i = 0; i<self.allModels.count; i++) {
        ...
        LiveListModel *hotModel = allModels[i];
...
- (void)clickBtn:(UIButton *)btn
{
    ...
    if (self.selectBlock){
        self.selectBlock(_allModels[btn.tag]);
    }
}
